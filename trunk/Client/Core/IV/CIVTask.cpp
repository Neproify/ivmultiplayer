...
{
	if(m_pTask)
		return g_pClient->GetClientTaskManager()->GetClientTaskFromGameTask(m_pTask->Clone());
...
const char * CIVTask::GetName()
{
	...
	{
		CLogFile::Printf("Got task with unknown name (VFTable: 0x%x, Type: %d (0x%x))", ((int**)m_pTask)[0], iType, iType);
		taskMap[iType] = true;
	}
...
CIVTask * CIVTaskComplex::CreateNextSubTask(CIVPed * pPed)
{
	...
	if(pTask)
		return g_pClient->GetClientTaskManager()->GetClientTaskFromGameTask(pTask->CreateNextSubTask(pPed->GetPed()));
...
CIVTask * CIVTaskComplex::CreateFirstSubTask(CIVPed * pPed)
{
	...
	if(pTask)
		return g_pClient->GetClientTaskManager()->GetClientTaskFromGameTask(pTask->CreateFirstSubTask(pPed->GetPed()));
...
CIVTask * CIVTaskComplex::ControlSubTask(CIVPed * pPed)
{
	...
	if(pTask)
		return g_pClient->GetClientTaskManager()->GetClientTaskFromGameTask(pTask->ControlSubTask(pPed->GetPed()));
