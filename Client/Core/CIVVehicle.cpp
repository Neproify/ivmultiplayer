...
void CIVVehicle::SetHazardLights(bool bState)
{
	// Do we have a valid vehicle pointer?
	IVVehicle * pVehicle = GetVehicle();

	//TODO: Fix hazard lights, already tested:
	//NATIVE_SET_VEH_HAZARDLIGHTS, doesn't work
	/*
	int iVehicle = GetScriptingHandle();
	bool bState = hState;
	int result = 0;
	_asm
	  {
	   ...
	   push iVehicle
	   mov ecx, result
	   call COffsets::FUNC_CVehicle__SetHazzardLights
	  }
	  doesn't work too */
	//SET_BIT and UNSET_BIT, doesn't work

	if(pVehicle)
	{
		if(bState)
		{
			pVehicle->m_byteFlags6 |= 4;
		} 
		else 
		{
			pVehicle->m_byteFlags6 &=~(4);
...
bool CIVVehicle::GetHazardLights(BYTE * getHazard)
{
	// Do we have a valid vehicle pointer?
	IVVehicle * pVehicle = GetVehicle();
	...
	if(pVehicle)
	{
		memcpy(&getHazard,&pVehicle->m_byteFlags6,sizeof(BYTE));
		return true;
	}
	return false;
}
